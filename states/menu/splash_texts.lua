...
-- Try to load tween library, but don't fail if it's not available
local tween = nil
local success, tween_lib = pcall(require, '/lib/tween/tween')
if success then
  tween = tween_lib
else
  print("Warning: Could not load tween library for splash_texts, using fallback")
end
...
function SplashText:new()
  ...
  local splashTextTween = nil
  if tween then
    splashTextTween = tween.new(dur, sp, {sc=1.2}, 'linear')
  end
  ...
  function splashText:update(dt)
    -- splashtext tween
    if tween and splashTextTween then
